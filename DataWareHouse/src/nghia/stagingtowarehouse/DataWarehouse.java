...
public class DataWarehouse {
	...
	public void run() {
		...
		if (connect_control == null) {
			...
			// NẾU KẾT NỐI THÀNH CÔNG --> 2.LẤY DỮ LIỆU CÓ STATUS ='TR'--> 3.TRẢ VỀ MỘT RESULTSET --> 4.1 LƯU LIỆU TRONG LIST VÀ ĐÓNG KẾT NỐI
			...
			if (connect_staging == null || connect_warehouse == null) {
				...
				for (MyConfig myConfig : listConfig) {
					// CHƯA TỒN TẠI DỮ LIỆU

					if (checkTableExist(connect_warehouse, myConfig.getStaging_table(), "datawarehouse") == 0) {
						//TẠO BẢNG
	...
	private ArrayList<MyConfig> getValuesFromConfig(Connection connection) {
		...
		String sql = "SELECT log.id_config, conf.staging_table, conf.variabless,conf.number_cols\r\n"
	...
	private void insertDatawarehouse(Connection connec_Stag, Connection connect_warehouse, MyConfig config) {
		...
		try {
			...
			while (result.next()) {
				String sqlInsert = "INSERT INTO " + config.getStaging_table() + " VALUES(";
