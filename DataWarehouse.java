...
public class DataWarehouse {
	...
	public static void main(String[] args) {
		...
		ExtractData dp = new ExtractData();
...
//		}
//	}
	public void ExtractToDB(ExtractData dp) {
		Configuration configuration = new Configuration(this.config_name);
		...
		String delim = configuration.getDelimeter();
		String column_list = configuration.getColumnList();
			...
			for (File file : listFile) {
				...
				if (file.getName().indexOf(file_type) != -1) {
					...
					if (values != null) {
						String table = "data_file";
						String file_status;
						int config_id = configuration.getConfigID();
						// time
						DateTimeFormatter dtf = DateTimeFormatter.ofPattern("yyyy/MM/dd HH:mm:ss");
						LocalDateTime now = LocalDateTime.now();
						String timestamp = dtf.format(now);
