...
public class DataProcess {
	...
	private String readLines(String value, String delim) {
		...
		for (int j = 0; j < countToken; j++) {
			...
			if (Pattern.matches(NUMBER_REGEX, token)) {
				...
				lines += (j == countToken - 1) ? "'z" + token.trim() + "')," : "'" + token.trim() + "',";
			}
			values += lines;
			lines = "";
		}
	...
	public boolean writeDataToBD(String field_name, String target_table, String values) throws ClassNotFoundException {
		if (cdb.insertValues(field_name, values, target_table))
			return true;
		return false;
	}
